...
OneSignal is disabled by default. Set `NEXT_PUBLIC_ONESIGNAL_APP_ID` and `ONESIGNAL_REST_API_KEY` to enable it. `NEXT_PUBLIC_ONESIGNAL_ALLOWED_ORIGINS` can restrict browser initialization further; when omitted, it uses the configured `NEXT_PUBLIC_APP_URL` origin. The OneSignal Web configuration must use the same deployed site URL.
...
`/api/calendar/sync` remains as a repair action. It no longer reads Google Calendar events directly. It finds the connected Recall calendar in Neon, incrementally processes changed Recall Calendar V2 events, and runs targeted repair for failed, missing, cancelled, or rescheduled bots. The `sync-recall-calendars-hourly` Inngest cron runs the same incremental repair once per hour for every local user with a connected Recall calendar.
